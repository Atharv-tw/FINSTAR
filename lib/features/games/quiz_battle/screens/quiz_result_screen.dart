...
class _QuizResultScreenState extends State<QuizResultScreen>
  ...
  String _getPerformanceMessage() {
    final percent = (widget.correctAnswers / widget.totalQuestions) * 100;
    if (percent == 100) return 'Perfect score! You are on fire.';
    if (percent >= 90) return 'Excellent work. Keep it up.';
    if (percent >= 80) return 'Great job. Solid knowledge.';
    if (percent >= 70) return 'Good progress. Keep learning.';
    if (percent >= 60) return 'Not bad. Review missed topics.';
    if (percent >= 50) return 'Getting there. Practice more.';
  ...
  Widget _buildGradeCard() {
    ...
    const darkColor = Color(0xFF393027);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.history, color: primaryColor),
              const SizedBox(width: 8),
              Text(
                'Answer Timeline',
                style: Theme.of(context).textTheme.titleLarge?.copyWith(
          ...
          Wrap(
            ...
            children: List.generate(
              ...
              (index) {
                ...
                final isWrong = outcome == AnswerOutcome.wrong;
  ...
  Widget _buildRewardsCard() {
    final isPerfect = widget.correctAnswers == widget.totalQuestions;
    final coinsToShow =
        _coinsEarned > 0 ? _coinsEarned : (widget.score ~/ 10 + (isPerfect ? 50 : 0));
    final xpToShow =
        _xpEarned > 0 ? _xpEarned : (widget.score ~/ 5 + (isPerfect ? 100 : 0));
    const Color primaryColor = Color(0xFF9BAD50);
    const Color lightColor = Color(0xFFB6CFE4);
    const Color darkColor = Color(0xFF393027);
        ...
        children: [
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () => context.go('/game/quiz-battle'),
